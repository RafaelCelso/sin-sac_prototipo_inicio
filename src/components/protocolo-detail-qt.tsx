...
import { Users, Monitor, MapPin, Send, Calendar, ChevronRight, Package, Tag, Barcode, Hash, FlaskConical } from "lucide-react";
...
interface ProtocoloDetailQTProps {
  id: string;
  cliente: string;
  criadoPor: string;
  status: string;
...
export function ProtocoloDetailQT({ id, cliente, criadoPor, status, territorio, dataCriacao, justificativa }: ProtocoloDetailQTProps) {
  const { isDark } = useTheme();

  const timeline: TimelineEvent[] = [
    { text: `Queixa Técnica ${id} registrada por ${criadoPor}`, time: "09:30", active: true },
    { text: `Encaminhado para análise técnica`, time: "10:45" },
    { text: `Laudo técnico em elaboração`, time: "14:00" },
  ];

  const comentarios: Comentario[] = [
    { autor: "Taiany", texto: "Produto apresentou defeito após 3 dias de uso. Cliente enviou fotos do problema.", data: "29/04/2026 - 09:30" },
    { autor: "Técnico", texto: "Análise preliminar indica falha no lote de produção. Aguardando laudo completo.", data: "29/04/2026 - 14:15" },
  ];
      ...
      <div>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className={`inline-flex rounded-full px-3 py-0.5 text-xs font-medium ${statusColor}`}>{status}</span>
        ...
        <div className="mt-2 grid grid-cols-2 gap-4">
          <div>
            ...
            <div className={`mt-0.5 flex items-center gap-1.5 text-sm font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}>
              <Monitor size={14} className="text-gray-500" />{criadoPor}
            </div>
          </div>
          <div>
            <p className="text-xs text-gray-500">Criado em</p>
            <div className={`mt-0.5 flex items-center gap-1.5 text-sm font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}>
              <Calendar size={14} className="text-gray-500" />{dataCriacao}
            </div>
          </div>
        </div>
        ...
        <div className="mt-3">
          ...
          <div className="flex items-center gap-2">
            <span className={`inline-flex rounded-full px-3 py-0.5 text-xs font-medium ${isDark ? "bg-white/10 text-gray-200" : "bg-[#EBEBEB] text-gray-800"}`}>Queixa Técnica</span>
            <ChevronRight size={14} className="text-gray-500" />
            <span className={`inline-flex rounded-full px-3 py-0.5 text-xs font-medium ${isDark ? "bg-white/10 text-gray-300" : "bg-[#EBEBEB] text-gray-700"}`}>Qualidade</span>
            <ChevronRight size={14} className="text-gray-500" />
            <span className={`inline-flex rounded-full px-3 py-0.5 text-xs font-medium ${isDark ? "bg-white/10 text-gray-300" : "bg-[#EBEBEB] text-gray-700"}`}>Defeito funcional</span>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-4">
          <div className={`rounded-lg border p-3 cursor-pointer hover:border-[#26B99D] hover:shadow-sm transition-all ${isDark ? "border-white/10" : "border-gray-200"}`}>
            ...
            <div className={`mt-1 flex items-center gap-1.5 text-sm font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}>
              <Users size={14} className="text-gray-500" />{cliente}
            </div>
