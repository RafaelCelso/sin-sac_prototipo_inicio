...
export function ProtocoloDetailNS({ id, cliente, criadoPor, status, territorio, dataCriacao, justificativa }: ProtocoloDetailNSProps) {
  ...
  const statusColor = status === "Revisão"
      ...
      <div>
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex flex-col items-end gap-1.5 shrink-0">
            <span className={`inline-flex rounded-full px-3 py-0.5 text-xs font-medium whitespace-nowrap ${statusColor}`}>
              {status}
            </span>
            <span className="inline-flex items-center gap-1 rounded-full bg-white/5 border border-white/10 px-3 py-0.5 text-xs font-medium text-gray-300 whitespace-nowrap">
              <MapPin size={12} />
              {territorio}
            </span>
          </div>
